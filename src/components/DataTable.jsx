...
const DataTable = ({marketData , isHome=false}) => {
  ...
  return (
    <div className={`w-full relative max-h-[260px] overflow-x-auto custom-scrollbar ${isHome && "border-b-4 border-[#265e04]"}  md:max-h-none`}>
        ...
        {marketData.map((market) => (
          <tr key={market.id} className="border-b border-[#c8ced3]">
            ...
            <td className="px-2 py-2 text-left">
              <a href="#" className="font-bold text-[#2789ce] no-underline">
                {market.eventName}
              </a>
              {market.matchTime && (
              ...
              {market.marketInfo.map((info, idx) => (
                <span
                  ...
                  className={`
                    ...
                    ${info === "S" ? "bg-gradient-to-b from-[#f26d1c] to-[#d14100]" : ""}
                    ${info === "BM" ? "bg-gradient-to-t from-[#14213d] to-[#315195] italic" : ""}
                    ${info === "F" ? "bg-gradient-to-b from-[#0a92a5] to-[#076875] ": ""}
                  `}
                >
                  {info}
                </span>
            ...
            {market.addToMultiMarkets && (
              <td className="px-4 py-2 text-center w-[25px]">
                <a href="#" title="Add to Multi Markets" className='text-[#7e97a7] cursor-pointer h-[6.666667vh] '>
                  <svg

...
const SportsProvider = ({ children }) => {
  ...
  const getAllSportsMatchList = async ()=>{
    ...
    try {
      ...
      setAllMatchList([...(response.data.t1 || []), ...(response.data.t2 || [])]);
      ...
      const matchData = await Promise.all(
        inPlayEtids.map(async ({ etid, name }) => {
          try {
            ...
            // Extract matches
            const allMatches = [...(response.data.t1 || []), ...(response.data.t2 || [])];
            ...
            return { etid, name, todayMatches: [], tomorrowMatches: [], inPlay: [] };
          }
        })
      );
  ...
  return (
    <SportsContext.Provider value={{ allSports, loading, error, getMatchListById, matchList , allMatchList , inPlayMatches , getInplayMatches}}>
      {children}
